...
from keras.layers import Dense, Dropout, Flatten, Add, Conv2D, MaxPooling2D, ZeroPadding2D, AveragePooling2D, \
...
parser.add_argument('--plot-history', action='store_true',
                    help='Determines if history of loss and accuracy should be plotted')
args = parser.parse_args()


def compiled_single_model(model_input_shape):
    input = Input(shape=model_input_shape)
    model_vgg16 = VGG16(weights='imagenet', include_top=False, input_tensor=input)
