...
def half_model():
    ...
    model = Conv2D(64, (6, 6))(input)
    model = BatchNormalization()(model)
    model = Activation('relu')(model)
    model = Conv2D(128, (3, 3), strides=(3, 3), activation='relu')(model)
...
def compiled_dual_model():
    ...
    merged_model = Conv2D(128, (3, 3), strides=(3, 3), activation='relu')(merged_model)
    merged_model = Conv2D(256, (3, 3), strides=(3, 3))(merged_model)
    merged_model = Dropout(.5)(merged_model)
    merged_model = Flatten()(merged_model)
    merged_model = Dense(256, activation='relu')(merged_model)
    merged_model = Dropout(.4)(merged_model)
